...
function createCloudTrailMessage(userId, event) {
    ...
    return JSON.stringify({
        "event": {
            ...
            "tag": "CloudTrailEvent",
            "summary": 'The user ' + userId + ' triggered event ' + detail.eventName + ' (' + 
                                    detail.eventType + ') via ' + detail.eventSource + '.',
            "severity": 1
        }
    });
}
...
function createEc2InstanceStateChangeMessage(event) {
    return JSON.stringify({
        "event": {
            ...
            "summary": 'The state of instance ' + event.detail['instance-id'] + ' has changed to ' + 
                                                    event.detail.state + ' at ' + event.time + '.',
            "severity": 1
        }
    });
}
...
function createAWSConfigEventMessage(event) {
    ...
    return JSON.stringify({
        "event": {
            "node": event.account+'-'+event.region,
            "tag": 'AWSConfig',
            "summary": 'A resource called ' + configItem.resourceName + ' of type '+ configItem.resourceType+' recorded a change type of: '+event.detail.configurationItemDiff.changeType,
            "severity": 1
        }
    });
}
...
exports.lambdaHandler = function(event, context, callback) {
    ...
    try{
        ...
        // dns.resolve(process.env.CONSOLE_FQDN, callback=(err, result) => {
        //     if (err) {
        //         console.error(err);
        //     } else {
        //         console.log(result);
        ...
        if(data){
            ...
            data.forEach(function(eData){
                const options = {
                    hostname: process.env.CONSOLE_FQDN,
                    port: 443,
